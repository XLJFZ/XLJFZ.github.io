...
type ImageItem = { src: string; alt: string; position?: string };
    ...
    <>
      <div className="space-y-3 md:space-y-5">
        {images.map((image, index) => (
          <button key={`${image.src}-${index}`} onClick={() => setActive(index)} className={`group block w-full overflow-hidden bg-neutral-200 ${index % 3 === 1 ? 'md:mx-auto md:w-[72%]' : ''}`} aria-label={`放大查看：${image.alt}`}>
