...
- 三维点击建筑取楼顶时，从建筑轮廓内选参考点，并读取 render_height；无有效楼高或轮廓时提示手动放置，不猜楼高。三维关闭普通地面标记拖动，二维保留拖动。楼顶高度为地图估算，用户可修正并加入相机架设高度。
...
- 地图提供二维平面与三维地形两种明确模式；三维楼顶模式使用原比例地形高程并调整俯仰角，二维模式须恢复俯视。地图容器必须保留真实高度并在尺寸变化时调用 `resize`，防止 MapLibre 样式覆盖后地图塌陷。
