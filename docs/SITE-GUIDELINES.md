...
最后核对：2026-09-20（对照近期成功部署及对应源码；核验范围见 [发布记录](RELEASE-HISTORY.md#2026-09-20-核对近期成功部署)）
...
- 公共页头必须由 `src/components/site-header.tsx` 统一渲染，并固定在视口顶部；首页与内页保持相同的毛玻璃背景、分隔线、导航状态和层级。页头后必须保留等高占位，防止固定定位遮挡正文。
- 页头毛玻璃是跨页面统一材质：`#151514/70` 底色 + `backdrop-blur-2xl` + `backdrop-saturate-150`，叠加自上而下的白色受光渐变（`from-white/[0.06]` → 透明）与顶缘 1px 内高光，底边框 `white/10`。70% 不透明度是刻意取值——让栏体色调主导，只透出一抹模糊光晕；**不要调回更透的档位**：在「远方的天气」等明暗对比强烈的卡片下方会出现左暗右亮的"半透窗"式不均匀（2026-09-12 已裁定）。
- 页头依赖根层叠上下文覆盖后续内容：**不要把 `<SiteHeader />` 放进带 `isolate`、`transform`、`filter`、`will-change` 的祖先容器内**。首页 hero 曾因 `isolate` 把页头困在局部层叠上下文，滚动时系列卡片整体绘制在页头之上（照片穿透毛玻璃），2026-09-12 修复。改动页头或 hero 结构后，应确认滚动状态下 `elementFromPoint` 在页头区域命中的是 `HEADER`。
...
- 字体使用 `next/font/google` 的 **Noto Sans SC（思源黑体）** 为主（`300` / `400` / `500` / `600`），叠加 **Geist** 作为拉丁字形。`body` 的字体栈把中文字体排在前面，而思源黑体自带拉丁区字形，因此中文与拉丁实际基本都由思源黑体渲染。**已评估并裁定不做字体子集化、不换系统字体栈**：跨平台排版一致性优先于传输体积，已接受首页首访约 `2547 KB`、字体占约 `84%` 的成本。不要再以体积或请求数数据为由重开此项。
- 本仓库是公开仓库，部署出的站点也是公开站点。**任何提交进仓库的图片都可以被任何人直接下载**——这是公开托管的固有属性，不是配置缺陷。因此资产策略的取舍不是“能不能防下载”，而是“把可获取的最高分辨率控制在什么水平”，判据见 §6。

### 首页首屏与系列衔接

- 首屏保持既有雪山作品、标题和地点，不改变图片资产；采用最小 `100svh` 的纵向布局，短屏允许内容自然撑高。
- 标题、介绍和浏览入口放在最大 `1600px` 的内容区内，手机单列、桌面左右分布；地点与“向下探索”位于内容流中的分隔线下方，禁止恢复固定 bottom 偏移的装饰横线。
- 遮罩保留山体中段细节，在下方文字区域逐渐加深；任何调整须同时检查桌面和手机的图片主体与文字可读性。
- “向下探索”使用原生 `#selected-series` 链接，目标区保留适配固定页头的滚动留白。保持减少动态效果设置生效，不增加循环跳动或强制滚动脚本。

### 首页合作区与公共页脚

- 首页合作区与公共页脚采用全宽背景、居中最大 `1600px` 内层版心，左右内边距为手机 20px、桌面 40px，与首页作品区对齐。
- 合作区保留“让影像保持呼吸”，独立展示拍摄方向，邮件入口使用有边框的主操作；手机单列、桌面双栏，不添加未经确认的合作经历或服务承诺。
- 页脚由 `src/components/site-footer.tsx` 统一渲染，分别组织署名、作品/关于导航、社交/复制邮箱和返回顶部；文字至少 12px，主要交互高度至少 44px，保留全站键盘焦点反馈。
- 页脚内层可以改变布局，但外层仍是 `main#top` 的直接子元素；保持自然文档流与既有贴底规则。共享修改必须检查首页、作品、关于、工具及 404 的长短页面，不能使用悬浮定位遮挡正文。

### 1.1 关于页版式与内容

- 关于页由 `src/app/about/page.tsx` 渲染，使用公共页头、页脚；版心最大 `1480px`，`md` 起为图文双栏，手机自然单列。
- 左侧复用「城市脉冲」中重庆建筑作品 `chongqing-zbz-9356`，图片、原始比例、替代文字与说明读取 `src/lib/portfolio.ts`，不另存原图、不裁切。图注提供进入原专题的链接；更换或移除该作品时必须同步检查关于页引用。
- 右侧依次为姓名、创作观点、个人介绍、工作与生活、拍摄方向、联系方式。创作观点按语义断行，避免手机上尾字单独成行；正文与辅助信息保持清晰灰阶。
- “工作与生活”使用用户确认的“上海 · 西安”；拍摄方向为“建筑 · 风光 · 旅行 · 编辑”。不得自行扩充履历、荣誉、客户或居住信息。
- 联系方式保留邮件链接、复制邮箱按钮和小红书入口；邮箱复制成功及失败应有可见反馈。修改后检查桌面与手机排版、照片完整性、专题链接和复制操作。

### 1.2 作品目录与辅助文字

- `/series/` 使用最大 `1600px` 版心，`md` 起为双列目录，四个系列按既定顺序形成 2×2 布局；手机单列。首页仍保留交替图文叙事，不跟随目录改成双列卡片。
- 目录卡片依次呈现封面、类别/年份/数量、中文标题、英文副标题、地点和箭头；同排封面与文字层级对齐，长地点自然换行。复用现有 16:10 封面及构图位置，不修改作品原始比例；`sizes` 必须反映双列实际宽度。
- 首页系列信息、目录与专题页的地点、年份和计数以至少 12px 显示，英文副标题使用 14px（专题顶部类别与英文组合为 12px）；常规辅助文字使用前景色 65% 左右，正文保持更高对比。编号可弱一级，但不恢复为 8–10px 的低对比小字。
- 画廊图注与灯箱地点/年份使用 12px；灯箱 EXIF 使用 11px、白色 65%，保持低于地点的层级。检查手机上图注、序号与 EXIF 不重叠，长章节导航仍可横向滚动；不改变图片配对、作品顺序或灯箱行为。
...
- 上述交叉检查只能用于**发现疑点**，不能单独用于**否定机身**。已确认的已知成因：作品在后期裁剪后再经手机端二次导出，会同时造成 ① `MakerNote` 被剥离 ② 像素比例不再是该机型原生比例 ③ 曝光有理数被改写成非常规形式（例如把 `1/320` 写成 `1000000/320000000`） ④ 写入手机厂商私有的 `UserComment`（如 `Oplus_*`）。因此即使同时出现这四项特征，仍可能确为该机身拍摄，应提请用户确认，而不得直接判定为「机型被改写」。
- 用户明确确认真实设备后，即按本节的证据优先级采信并生效，**不得再以「缺少 `MakerNote`」或「像素比例不符」为由反复质疑或删改已确认的 `camera` 字段**。
...
```text
public/portfolio-previews/           画廊响应式预览，脚本生成；这是唯一随站点发布的作品图
...
- 机位与目标点分别填写离地高度和地面海拔；几何计算、三维投影统一使用两者之和。地图点击尝试读取地形海拔，无数据明确按 0m 并允许手动补填。
- 显示水平距离、高差、直线距离与俯仰角；焦段覆盖使用直线距离，假设相机瞄准目标中心、目标横向垂直于视线。PNG 计划卡须包含两端高度、直线距离与俯仰角。
- 三维楼顶模式须按地图相机矩阵投影真实 XYZ 标记、离地辅助线和两点连线；不能用固定像素偏移冒充高度。标注始终显示，不表示视线无遮挡；二维扇形与光线分类仍为平面参考。
- 楼顶吸附选项须随工具栏自然换行，手机端不得使用固定顶部偏移遮挡二维／三维按钮。切换地图模式时保留高度，选点不能自动旋转地图。
- 三维点击建筑取楼顶时，从建筑轮廓内选参考点，优先读取 `render_height`，缺失时使用有效的 `height`；无有效楼高或轮廓时提示手动放置，不猜楼高。三维关闭普通地面标记拖动，二维保留拖动。楼顶高度为地图估算，用户可修正并加入相机架设高度。
- 三维投影实现位于 `src/lib/planner-map-overlay.ts`，使用 Mercator 投影及地图当前相机矩阵；楼顶参考点和高度校验位于 `src/lib/planner-rooftops.ts`，须处理凹形轮廓与中庭。`tests/planner-rooftops.test.mjs` 验证高度投影、相机后方点隐藏、轮廓内参考点和无效楼高拒绝。
- 高差计算由 `tests/sightline.test.mjs` 验证等高、仰拍、俯拍、共同高度基准平移、垂直视线、两点重合与焦段覆盖。
...
- 地图提供“二维地图”与“三维楼顶”两种明确模式；三维楼顶模式使用原比例地形高程并调整俯仰角，二维模式须恢复俯视。地图容器必须保留真实高度并在尺寸变化时调用 `resize`，防止 MapLibre 样式覆盖后地图塌陷。
...
- 当前工作流使用 `actions/checkout@v7`、`actions/setup-node@v7`、`actions/configure-pages@v6`、`actions/upload-pages-artifact@v5`、`actions/deploy-pages@v5`；应用构建仍指定 Node 22，不能从 Action 的主版本推断应用 Node 版本。版本以 `.github/workflows/pages.yml` 为准，后续升级应同步本节并核对该提交的部署结果。
